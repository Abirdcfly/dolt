...
func MigrateCommit(ctx context.Context, cm *doltdb.Commit, _, new *doltdb.DoltDB, prog Progress) error {
	...
	cs := tree.HackChunkStoreFromNodeStore(new.NodeStore())

	migratedCm, err := datas.NewCommitForValue(ctx, cs, new.ValueReadWriter(), new.NodeStore(), value, opts)
	if err != nil {
		return err
	}
...
func MigrateTable(ctx context.Context, table *doltdb.Table, new *doltdb.DoltDB) (*doltdb.Table, error) {
	...
	newSet, err := MigrateIndexSet(ctx, sch, oldSet, new)
	if err != nil {
		return nil, err
	}
...
func MigrateIndexSet(ctx context.Context, sch schema.Schema, oldSet durable.IndexSet, new *doltdb.DoltDB) (durable.IndexSet, error) {
