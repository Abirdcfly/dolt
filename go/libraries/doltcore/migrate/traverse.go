...
func TraverseBranchHistory(ctx context.Context, r ref.BranchRef, old, new *doltdb.DoltDB, prog Progress) error {
	...
	if err = TraverseCommitHistory(ctx, cm, old, new, prog); err != nil {
		return err
	}
...
func TraverseCommitHistory(ctx context.Context, cm *doltdb.Commit, old, new *doltdb.DoltDB, prog Progress) error {
	for {
		...
		if idx < 0 {
			...
			if err = MigrateCommit(ctx, cm, old, new, prog); err != nil {
				return err
			}
