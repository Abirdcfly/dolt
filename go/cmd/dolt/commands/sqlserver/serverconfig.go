...
func ValidateConfig(config ServerConfig, protocol string) error {
	if protocol == "tcp" {
...
func ConnectionString(config ServerConfig) string {
	...
	if runtime.GOOS == "windows" || config.Host() != "localhost" || config.Port() != -1 {
		...
		return fmt.Sprintf("%v:%v@tcp(%v:%v)/", config.User(), config.Password(), host, port)
	}
	return fmt.Sprintf("%v:%v@unix(%v)/", config.User(), config.Password(), defaultSocketFileLocation)
}
